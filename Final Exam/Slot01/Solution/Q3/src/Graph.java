...
/*
    void f1()-Perform depth-first traversal (to the file f1.txt) from the vertex i=3 (the vertex D) but display 5 vertices from the 3rd vertex to the 7th vertex only. 
    Hint: copy depth(...) to depth2(...) and modify the latter one. Content of the output file f1.txt must be:
    D A B E H I C G F
    B E H I C

    Không làm
    void f2() - Apply the Dijkstra's shortest path algorithm to find (1) the shortest path from vertext 0 (A) to vertex 6 (G), then (2) from vertex 1 (B) to vertex 5 (F). 
    Write 3 lines to the file f2.txt: line 1 contains the last 3 vertices selected into the set S with their labels in (1), line 2 contains the 1st, 
    3rd and last vertices in shortest path (1), line 3 contains vertices in shortest path (2). (Note that in the weighted matrix, the value 99 is considered as infinity). 
