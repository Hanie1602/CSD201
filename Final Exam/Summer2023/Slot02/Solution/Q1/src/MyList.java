...
public class MyList {
 ...
 Example: with the content given in the file 'data.txt', the content of 'f1.txt' after insertion 
    ...
    void f2() throws Exception {
        ...
        * Update: sử dụng hàm set để thay đổi tất cả các node trong dslk nếu tên là Q2 or Q9 thì đổi lại thành TD và tăng giá trị tài sản lên 1
        * 
    ...
    void f3() throws Exception {
       ...
       //duyệt tất cả phần tử trong danh sách để tìm max area
        ...
        for(int i=0; i<n-1; i++) {
            for(int j=i+1; j<n; j++) {
                ...
                if(pi.getInfo().price < pj.getInfo().price) {
                    //set price 
                    int temp=pi.getInfo().price; 
                     pi.getInfo().price=pj.getInfo().price; 
                     pj.getInfo().price=temp;
                    
                    //set distinct 
                     String tempStr = pi.getInfo().district;
                     pi.getInfo().district = pj.getInfo().district;
                     pj.getInfo().district = tempStr;
                     
                     //set area
                     int temp3 =pi.getInfo().area; 
                     pi.getInfo().area=pj.getInfo().area; 
                     pj.getInfo().area=temp3;
                }
            }
        }
    }
